...
import { generateAISuggestions, detectSlop, type AISuggestion, type SlopIndicator } from '@/lib/aiSuggestions'
  ...
  .handler(async ({ data }): Promise<AnalyzeResponse | ErrorResponse> => {
    ...
    try {
      ...
      return {
        ...
        lighthouse: lighthouseData as Awaited<ReturnType<typeof analyzeLighthouse>>,
        aiSuggestions,
        slopIndicators,
      }
    } catch (error) {
