...
function parsePSIResult(result: unknown): LighthouseMetrics {
  try {
    ...
    const getAudit = (id: string) => audits[id] as Record<string, unknown> | undefined

    const netReq = (() => {
      const details = getAudit('network-requests')?.details as { items?: unknown[] } | undefined
    ...
    const opps = Object.values(audits)
      ...
      .map((a) => ({
        ...
        savingsMs: Math.round((a.details as { overallSavingsMs: number }).overallSavingsMs),
      }))
      .sort((a, b) => b.savingsMs - a.savingsMs)
      .slice(0, 5)

    const largestAssets: Array<{ url: string; size: number; type: string }> = []
    const resourceSummary = getAudit('resource-summary')?.details as { items?: unknown[] } | undefined
    const assetDetails = resourceSummary?.items || []
    for (const item of assetDetails) {
      const i = item as { url?: string; transferSize?: number; resourceType?: string }
      if (i.url && i.transferSize && i.resourceType) {
        largestAssets.push({ url: i.url, size: i.transferSize, type: i.resourceType })
      }
    }
    ...
    const thirdParty = getAudit('third-party-summary')?.details as { items?: unknown[] } | undefined
    const thirdPartyDetails = thirdParty?.items || []
    for (const item of thirdPartyDetails) {
      const i = item as { entity?: string; requestCount?: number; transferSize?: number }
      if (i.entity && i.requestCount && i.transferSize) {
        thirdPartySummary.push({ domain: i.entity, requests: i.requestCount, bytes: i.transferSize })
      }
    }
    ...
    const screenshot =
      (getAudit('final-screenshot')?.details as { data?: string } | undefined)?.data || null
    ...
    return {
      ...
      ttfb: toMs(
        getAudit('server-response-time')?.numericValue ?? getAudit('time-to-first-byte')?.numericValue
      ),
      totalByteWeight: (getAudit('total-byte-weight')?.numericValue as number) ?? null,
...
async function runPSIAnalysis(url: string, strategy: 'mobile' | 'desktop'): Promise<LighthouseMetrics> {
  try {
    ...
    console.error(`PSI analysis error (${strategy}):`, (e as Error)?.message || e)
    return EMPTY
  }
}
