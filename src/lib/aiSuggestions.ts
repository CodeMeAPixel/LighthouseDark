...
export function detectSlop(seo: SEOData | null): SlopIndicator[] {
	...
	if (seo.h1Count === 0) {
		indicators.push({
			type: "missing-h1",
			evidence: "No H1 heading",
			severity: "high",
		});
	} else if (seo.h1Count > 1) {
		indicators.push({
			type: "multiple-h1",
			evidence: `${seo.h1Count} H1 tags`,
			severity: "medium",
		});
	}
...
function compressForAI(
	...
	if (seo) {
		...
		if (seo.canonical && !seo.canonicalIsSelf)
			issues.push({ type: "canonical-mismatch" });
		if (!seo.viewport) issues.push({ type: "no-viewport" });
		if (seo.viewport && !seo.viewportIsResponsive)
			issues.push({ type: "bad-viewport" });
	...
	if (lighthouse) {
		const addMetricIssue = (
			...
			if (metrics.totalByteWeight && metrics.totalByteWeight > 3000000) {
				issues.push({
					type: "heavy-page",
					value: Math.round(metrics.totalByteWeight / 1024),
					scope,
				});
			}
			...
			if (metrics.opportunities.length > 0) {
				...
				if (topOpp.savingsMs > 500) {
					issues.push({
						type: "opportunity",
						value: `${topOpp.id}:${topOpp.savingsMs}ms`,
						scope,
					});
				}
			}
			...
			// Third-party bloat
			const totalThirdParty = metrics.thirdPartySummary.reduce(
				(sum, tp) => sum + tp.bytes,
				0,
			);
			if (totalThirdParty > 500000) {
				issues.push({
					type: "3p-bloat",
					value: Math.round(totalThirdParty / 1024),
					scope,
				});
			}
		};
...
function formatIssuesCompact(issues: CompressedIssue[]): string {
	...
	if (grouped.desktop.length)
		parts.push(`Desktop: ${grouped.desktop.join(", ")}`);
...
function generateFallbackSuggestions(
	...
	const issueMap: Record<
		string,
		Omit<AISuggestion, "name"> & { name: string }
	> = {
		"no-title": {
			name: "Add a Title Tag ASAP",
			message:
				"Your page has no title tag. This is SEO 101 — search engines need this to understand your page.",
			impact: "high",
			effort: "low",
			category: "seo",
		},
		"no-meta-desc": {
			...
			message:
				"No meta description means Google will pick random text from your page. Take control of your snippet.",
			impact: "high",
			effort: "low",
			category: "seo",
		},
		"slow-lcp": {
			...
			message:
				"Your largest content takes too long to paint. Optimize images, add preload hints, or use a CDN.",
			impact: "high",
			effort: "medium",
		...
		"high-cls": {
			...
			message:
				"Elements are jumping around as your page loads. Add explicit dimensions to images and embeds.",
			impact: "high",
			effort: "medium",
			category: "performance",
		},
		"missing-alt": {
			...
			message:
				"Images without alt text hurt accessibility and SEO. Describe what each image shows.",
			impact: "medium",
			effort: "low",
		...
		"no-schema": {
			...
			message:
				"No JSON-LD schema found. Add structured data to get rich results in search.",
			impact: "medium",
			effort: "medium",
			category: "seo",
		},
	...
	for (const s of slop) {
		...
		if (s.type === "generic-title" || s.type === "generic-description") {
			suggestions.push({
				...
				message:
					"Your content looks auto-generated or uses placeholders. Write unique, specific copy.",
				impact: "high",
				effort: "medium",
				category: "content",
			});
		...
		if (s.type === "ai-content-markers") {
			suggestions.push({
				...
				message:
					"Your content has telltale AI-generated phrases. Edit for authenticity and brand voice.",
				impact: "medium",
				effort: "medium",
				category: "content",
			});
