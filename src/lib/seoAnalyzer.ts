...
export async function analyzeSEO(url: string): Promise<SEOData | undefined> {
	...
	try {
		...
		if (!res.ok) {
			if (res.status === 403) {
				throw new Error(
					"Access Forbidden (403): The target site is blocking the analysis. This often happens with Cloudflare or bot protection.",
				);
			}
		...
		const toAbsolute = (v: string | null) =>
			v ? new URL(v, baseUrl).href : null;
		...
		const viewportContent =
			viewportMeta?.getAttribute("content")?.toLowerCase() || "";
		const viewportIsResponsive =
			/width\s*=\s*device-width/.test(viewportContent) &&
			/initial-scale\s*=/.test(viewportContent);

		const hasAnyIcon = !!document.querySelector(
			"link[rel~='icon'], link[rel='shortcut icon'], link[rel='mask-icon']",
		);

		const jsonLdCount = document.querySelectorAll(
		...
		const robotsCombined =
			`${getMeta(["robots"]) || ""} ${res.headers.get("x-robots-tag") || ""}`.toLowerCase();
		...
		const normalizeForCompare = (u: string) => {
			try {
				const a = new URL(u);
				const p = a.pathname.endsWith("/")
					? a.pathname.slice(0, -1)
					: a.pathname;
		...
		const canonicalIsSelf = !!(
			canonicalAbs &&
			normalizeForCompare(canonicalAbs) === normalizeForCompare(finalUrl)
		);
		...
		const preconnectCount = document.querySelectorAll(
			"link[rel='preconnect']",
		).length;
		...
		const titleLength =
			titleTag?.length || ogTitle?.length || h1Fallback?.length || null;
		const descriptionLength =
			getMeta(["description"])?.length ||
			getMeta(["og:description"])?.length ||
			null;
