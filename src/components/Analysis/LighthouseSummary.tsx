...
import type { LighthouseData, LighthouseMetrics } from '@/lib/lighthouseAnalyzer'

interface LighthouseSummaryProps {
  data: LighthouseData
}

function ScoreGauge({ score, label }: { score: number | null | undefined; label: string }) {
  ...
  return (
    ...
    >
      ...
      {label && (
        <span className="mt-2 text-xs text-light11 dark:text-dark11 font-medium">{label}</span>
      )}
    </motion.div>
  )
}
    ...
    >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          />
          <MetricTile label="CLS" value={metrics.cls} threshold={{ good: 0.1, poor: 0.25 }} isCLS />
          <MetricTile
            label="TBT"
            value={metrics.tbt}
            unit="ms"
      ...
      {/* Additional Metrics */}
      <CollapsibleSection title="Additional Metrics" icon={null} defaultOpen={false}>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <MetricTile
            label="FCP"
            value={metrics.fcp}
          ...
          />
          <MetricTile label="Total Bytes" value={metrics.totalByteWeight} unit=" B" />
      ...
      {metrics.opportunities.length > 0 && (
        <CollapsibleSection title="Opportunities" icon={null} defaultOpen={true}>
          <div className="space-y-2">
            {metrics.opportunities.map((opp, i) => (
              ...
              >
                ...
                <span className="text-sm font-medium text-amber-500">~{opp.savingsMs}ms</span>
              </div>
            ))}
          </div>
        </CollapsibleSection>
