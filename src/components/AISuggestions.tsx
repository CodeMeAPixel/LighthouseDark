...
const getEffortStyles = (effort: AISuggestion['effort']) => {
  switch (effort) {
    case 'low':
      return { label: 'Quick Fix', color: 'text-emerald-500', icon: <Zap className="h-3 w-3" /> }
    case 'medium':
      return { label: 'Some Work', color: 'text-amber-500', icon: <Wrench className="h-3 w-3" /> }
    case 'high':
      return { label: 'Major Effort', color: 'text-red-400', icon: <Clock className="h-3 w-3" /> }
  }
}
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="space-y-4">
        {suggestions.map((suggestion, index) => {
          ...
          const impact = suggestion.impact || (index < 2 ? 'high' : index < 4 ? 'medium' : 'low')
          ...
          return (
            ...
            >
              <div className={`absolute left-0 top-0 h-full w-1 ${impactStyles.bg.replace('/10', '')}`} />
              ...
              <div className="p-5 pl-6">
                ...
                <h4 className="font-semibold text-light12 dark:text-dark12 mb-2">{suggestion.name}</h4>
                <p className="text-sm leading-relaxed text-light11 dark:text-dark11">{suggestion.message}</p>

                {/* Bottom row: Effort indicator and action hint */}
                <div className="mt-4 flex items-center justify-between">
                  <div className={`flex items-center gap-1.5 text-xs ${effortStyles.color}`}>
                    {effortStyles.icon}
