...
import { Home, ArrowLeft, Flame, Compass, MapPin, Sparkles } from 'lucide-react'
    ...
    <div className="relative flex min-h-[80vh] flex-col items-center justify-center px-4 py-16">
      ...
      >
        ...
        >
          ...
          >
            <Flame className="h-28 w-28 text-orange-500 drop-shadow-[0_0_40px_rgba(249,115,22,0.5)] md:h-36 md:w-36" strokeWidth={1.5} />
            <motion.div
              className="absolute inset-0 flex items-center justify-center"
              animate={{ opacity: [0.5, 1, 0.5] }}
              transition={{ duration: 1.5, repeat: Infinity }}
            >
              <Flame className="h-28 w-28 text-yellow-400/50 blur-sm md:h-36 md:w-36" strokeWidth={1.5} />
            </motion.div>
          </motion.div>
        ...
        <motion.p
          ...
          transition={{ delay: 0.3 }}
        >
          This page was burned to a crisp. It either never existed, moved somewhere else, or got a little too close to the flame.
        </motion.p>
        ...
        >
          <div className="glass-surface rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                { icon: Home, text: 'Head back home to analyze a new site', to: '/' },
                { icon: MapPin, text: 'Check out the About page', to: '/about' },
                { icon: Sparkles, text: 'Read our FAQs for guidance', to: '/faqs' },
              ].map((item) => (
                <Link
                  key={item.to}
                  to={item.to}
